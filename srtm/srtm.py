...
Example Query all tiles, normalize the range and generate corresponding color tiles for all elevations within Longitude Latiutude 34N 2W to 35N 1E
    ...
    $ python srtm.py            \ 
        SRTM_HGTs/              \   # specify that dataset can found in the local dir SRTM_HGTs
...
def get_tile_filename(latlong, dataset_type):
    if dataset_type == 'srtm1':         # format [NS]Y[EW]XXX.hgt
        ...
        return f'{prefix_lat}{lat}{prefix_long}{long:03}.hgt'
...
def export_raster_color(raster, out_filename, minmax):
    '''
...
def export_raster_grid_colors(raster_grid, minmax, prefix):    
    # Create a 8-bit PNG that contains color normalized to entire raster_grid min and max. 
    for y in range(raster_grid.shape[0]):
        for x in range(raster_grid.shape[1]):
                out_filename = f'{prefix}_x{x}_y{y}.png'
...
def exec_args():
    ...
    if args.sigma > 0:
        ...
        sys.stdout.flush()
        smooth_raster_grid(raster_grid, sigma)
    ...
    if args.colors : 
        ...
        export_raster_grid_colors(raster_grid, minmax, args.colorprefix)
